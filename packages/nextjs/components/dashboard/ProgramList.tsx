        ...
        <div className="row">
            ...
            {programs.map((program, index) => (
                <div key={index} className="col-md-6 mt-2">
                    <div
                        className="bg-white job-box job-primary d-md-flex align-items-center border-0 shadow rounded p-4 position-relative">
                        <div className="ms-md-4 mt-4 mt-sm-0">
                            <a onClick={(event) => {
                                event.preventDefault()
                                programClicked(program.id)
                            }} href="#" className="title text-dark h5">{program.name}</a>

                            <ul className="list-unstyled mb-0 mt-2">
                                <li className="d-lg-inline text-muted h6 mb-0 me-lg-2">
                                    <i className="uil uil-bookmark icons"/>{program.description}
                                </li>
                            </ul>
                            ...
                            <div className="mt-2">
                                ...
                                <div className="mt-2 d-flex justify-content-between">
                                    <span
                                        className={`badge text-bg-soft d-flex align-items-center text-capitalize ${getClassProgressStatus(program.status)}`}>
                                        {program.status}
                                    </span>
                                    {!['completed', 'draft'].includes(program.status) && <a href="" className="text-primary p-1 px-2 shadow rounded me-3">
